/**
 * Deploy the Taxi CRM stack to a VPS over SSH (native Node + PM2 + nginx).
 *
  ...
  if (!fs.existsSync(envPath)) {
    console.error(
      "\nMissing .env in the project root.\n" +
        "  cp .env.example .env\n" +
        "  Then set BOT_TOKEN, DOMAIN, PUBLIC_URL, and VPS MySQL values.\n",
    );
    process.exit(1);
  }
...
function buildServerEnv(source) {
  ...
  const server = {
    ...
    TELEGRAM_SUPERADMIN_ID: source.TELEGRAM_SUPERADMIN_ID?.trim(),
    ...
    LOCAL_DB_PORT: source.LOCAL_DB_PORT?.trim() || "3306",
    NODE_ENV: "production",
    API_PORT: source.API_PORT?.trim() || "3000",
    CORS_ORIGINS: source.CORS_ORIGINS?.trim() || publicUrl,
    RUN_SCHEDULER: source.RUN_SCHEDULER_VPS?.trim() || "true",
    REMINDER_DAYS_BEFORE: source.REMINDER_DAYS_BEFORE?.trim() || "7",
...
function prepareAppEnv(source) {
  requireKeys(source, ["BOT_TOKEN", "TELEGRAM_SUPERADMIN_ID"]);
  const mysql = vpsMysql(source);
  requireKeys(
...
function validateProductionEnv(source) {
  requireKeys(source, ["BOT_TOKEN", "TELEGRAM_SUPERADMIN_ID", "PUBLIC_URL", "DOMAIN"]);
  const mysql = vpsMysql(source);
  requireKeys(
    { MYSQL_USER: mysql.user, MYSQL_PASSWORD: mysql.password, MYSQL_DATABASE: mysql.database },
    ["MYSQL_USER", "MYSQL_PASSWORD", "MYSQL_DATABASE"],
...
function configureSsl(env) {
  console.log("\n🔒 Requesting HTTPS certificate (certbot)…");
  ssh(
...
function printDone(env) {
  ...
  console.log(`  Health:    ${env.PUBLIC_URL.replace(/\/$/, "")}/health`);
  console.log(`  Bot:       pm2 logs taxi-bot`);
  console.log(`  API:       pm2 logs taxi-api`);
...
switch (step) {
  case "setup": {
    ...
    ssh(`mkdir -p '${appDir}'`);
    setupMysql(env);
    console.log("\n✅ MySQL user and database ready on the VPS.\n");
    break;
  }
  case "app": {
    const env = prepareAppEnv(sourceEnv);
    ...
    remoteBuildAndStart();
    console.log("\n✅ App built and running under PM2 (taxi-api, taxi-bot).");
  ...
  default: {
    ...
    configureNginx(env);
    console.log(
      "\nℹ️  HTTPS: run `npm run deploy:vps -- ssl` after DNS points to this server.\n",
    );
    printDone(env);
    break;
  }
